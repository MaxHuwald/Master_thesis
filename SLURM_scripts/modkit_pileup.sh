#!/bin/bash
#SBATCH --job-name=modkit_pileup_eU  # Job name
...
# Run Modkit
"$MODKIT" pileup "$BAM"  $OUT_DIR/"$NAME".pseU.bed  \
--filter-threshold A:0.8 --filter-threshold T:0.8 --filter-threshold C:0.8 --filter-threshold G:0.8 --mod-thresholds 17802:0.98 --motif T 0 --ref $REF --log-filepath $OUT_DIR/"$NAME".pseU.log
