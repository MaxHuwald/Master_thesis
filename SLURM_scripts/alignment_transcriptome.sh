#!/bin/bash
#SBATCH --job-name=alignment_transcriptome  # Job name
...
OUT=/project/sysviro/users/Max/analyses/Conditions/transcriptome
VERSION=1.1.1
GENOME=/project/sysviro/users/Max/Genome/HSV2-st333-LS480640_transcripts_v5.fa
...
samtools sort -o $OUT/"$COND".trimAdapters.dorado."$VERSION".untrimmed.filtered.aligned_transcriptome.primary.bam -
...
samtools view -b -F2324 $OUT/"$COND".trimAdapters.dorado."$VERSION".untrimmed.filtered.aligned_transcriptome.primary.bam > $OUT/Visualisation/"$COND".trimAdapters.dorado."$VERSION".untrimmed.filtered.aligned_transcriptome.primary.fwd.bam
...
samtools view -b $OUT/Visualisation/"$COND".trimAdapters.dorado."$VERSION".untrimmed.filtered.aligned_transcriptome.primary.fwd.bam | genomeCoverageBed -ibam stdin -bg -split > $OUT/Visualisation/"$COND".trimAdapters.dorado."$VERSION".untrimmed.filtered.aligned_transcriptome.primary.fwd.bedgraph
...
bamToBed -bed12 -i $OUT/Visualisation/"$COND".trimAdapters.dorado."$VERSION".untrimmed.filtered.aligned_transcriptome.primary.fwd.bam > $OUT/Visualisation/"$COND".trimAdapters.dorado."$VERSION".untrimmed.filtered.aligned_transcriptome.primary.fwd.bed
