#!/bin/bash
#SBATCH --job-name=guppy_alignment  # Job name
...
# |                                           VARIABLES                                            |
# |________________________________________________________________________________________________|
NAME=HSV2-ARPE19-10h-1-hac.v6.5.7
IN=/project/sysviro/data/GuppyBasecalling/HSV2-ARPE19-10h-1-hac.v6.5.7.fastq
OUT=/project/sysviro/users/Max/analyses/Guppy_data
...
minimap2 -ax splice -k14 -uf -y --secondary=no $GENOME $IN | \
samtools view -b - > $OUT/"$NAME".aligned.bam
samtools view -b -F2308 $OUT/"$NAME".aligned.bam > $OUT/"$NAME".aligned.primary.bam


samtools view -b -f16 $OUT/"$NAME".aligned.primary.bam > $OUT/Visualisation/"$NAME".aligned.primary.rev.bam

samtools view -b -F2324 $OUT/"$NAME".aligned.primary.bam > $OUT/Visualisation/"$NAME".aligned.primary.fwd.bam

samtools view -b $OUT/Visualisation/"$NAME".aligned.primary.rev.bam | genomeCoverageBed -ibam stdin -bg -split > $OUT/Visualisation/"$NAME".aligned.primary.rev.bedgraph

samtools view -b $OUT/Visualisation/"$NAME".aligned.primary.fwd.bam | genomeCoverageBed -ibam stdin -bg -split > $OUT/Visualisation/"$NAME".aligned.primary.fwd.bedgraph

bamToBed -bed12 -i $OUT/Visualisation/"$NAME".aligned.primary.rev.bam > $OUT/Visualisation/"$NAME".aligned.primary.rev.bed

bamToBed -bed12 -i $OUT/Visualisation/"$NAME".aligned.primary.fwd.bam > $OUT/Visualisation/"$NAME".aligned.primary.fwd.bed
