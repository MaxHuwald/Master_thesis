...
minimap2 -ax splice -k14 -uf -y --secondary=no $GENOME - | \
samtools view -bS - | \
samtools sort - > $OUT/"$NAME".sup-allMods."$COND".trimAdapters.dorado."$VERSION".filtered.aligned.bam

samtools view -b -F2308 $OUT/"$NAME".sup-allMods."$COND".trimAdapters.dorado."$VERSION".filtered.aligned.bam | \
samtools sort - > $OUT/"$NAME".sup-allMods."$COND".trimAdapters.dorado."$VERSION".filtered.aligned.primary.bam
